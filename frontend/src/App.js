...
import { contractAbiForCrowdFund,
         ...
         contractAbiForVotingToken,contractAbiForCropTrading,contractAbiForCrowdFunding,contractAbiForMicroFinance,contractAddressCrowdFunding
        ,contractAddressForCropTrading,contractAddressMicroFinance } from "./abi/contractInstance";
...
import CropTrading from './CropTrading.jsx';
...
function App() {
  ...
  const connectContract = async (signer) => {
    if (signer) {
      ...
      const cropTradingContract = new ethers.Contract(contractAddressForCropTrading, contractAbiForCropTrading,signer);
      const crowdFundingContract = new ethers.Contract(contractAddressCrowdFunding,contractAbiForCrowdFunding,signer);
      const microfinanceContract = new ethers.Contract(contractAddressMicroFinance,contractAbiForMicroFinance,signer);
      setContractBundle({
        ...
        cropTradingContract:cropTradingContract,
        crowdFundingContract:crowdFundingContract,
        microfinanceContract:microfinanceContract
      })
    ...
    <div className="px-8 md:px-16">
      ...
      {provider ? (
        ...
        <Route path="/donate" element={<Donate signer={signer} contractBundle={contractBundle}   />} />
          ...
          <Route path="/croptrading" element={<CropTrading signer={signer} contractBundle={contractBundle} />} />
        </Routes>
      {/* ) : (    */}
          {showConnectButton ? (
            <div>
